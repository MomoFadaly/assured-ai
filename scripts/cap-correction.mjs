...
await mkdir('/tmp/correction-cap', { recursive: true });
...
const BEATS = [
  { ds: 0.860, label: 'a-pre-correction' },
  { ds: 0.864, label: 'b-strikethrough-drawing' },
  { ds: 0.868, label: 'c-strike-done-typing-starts' },
  { ds: 0.872, label: 'd-typing-mid' },
  { ds: 0.880, label: 'e-correction-complete' },
  { ds: 0.890, label: 'f-scene8-full-with-correction' },
  { ds: 0.900, label: 'g-scene8-late' },
];
for (const beat of BEATS) {
  ...
  await p.screenshot({ path: `/tmp/correction-cap/${beat.label}.jpg`, type: 'jpeg', quality: 88, fullPage: false });
